...
void GameSelectScene::addDebugGameButton(Node *parent, std::string gameName,
                                         Vector<ui::Button *> &iconVector) {
    ...
    btn->addTouchEventListener(
            [gameName, this](Ref *pSender, ui::Widget::TouchEventType eEventType) {
...
void GameSelectScene::addGameButton(Node *parent, std::string levelID, int day, int gameIndex,
                                    Vector<ui::Button *> &iconVector) {
    ...
    if (isAvailable || _bird->getCategoryLevel() == CoopScene::LEVEL_SPECIAL_COURSE) {
        btn->addTouchEventListener([levelID, day, gameIndex, this](Ref *pSender,
            ...
            switch (eEventType) {
                ...
                case ui::Widget::TouchEventType::ENDED:
                    StrictLogManager::shared()->dailyGameChoice_ChooseDailyGame(levelID, day,
                                                                                gameIndex);

                    cover->setVisible(false);
                    _transitionBegins = true;
                    SoundEffect::buttonEffect().play();
                    CCAppController::sharedAppController()->startCurriculumGame(levelID, day,
                                                                                gameIndex);
                    break;
    ...
    if (gameInfo.gameName != "EggQuizLiteracy" && gameInfo.gameName != "EggQuizMath" &&
        gameInfo.gameName != "Book" && gameInfo.gameName != "Comprehension" &&
        gameInfo.gameName != "Video" && gameInfo.gameName != "BookWithQuiz") {
        ...
        Node *levelNumber = Sprite::create(
                StringUtils::format("icons/game_level_circle_%d.png", gameInfo.gameLevel));
        if (!levelNumber) {
            levelNumber = TodoUtil::createLabel(TodoUtil::itos(gameInfo.gameLevel), 62, Size::ZERO,
                                                "fonts/mukta-bold.ttf", Color4B::WHITE);
...
void
GameSelectScene::addDayGameButtons(std::string levelID, int day, Vector<ui::Button *> &iconVector) {
    auto cur = CurriculumManager::getInstance()->findCurriculum(levelID);
    auto dayCur = cur->getDayCurriculum(day);
            ...
            if (dayCur) {
                ...
                if (dayCur->isEggQuiz && !dayCur->isMiniQuiz) {
                    if (UserManager::getInstance()->getPretestProgressType(levelID) !=
                        ...
                        UserManager::getInstance()->setPretestProgressType(levelID,
                                                                           PretestProgressType::pass);
                    }
                }

                if (_bird->getCategoryLevel() == CoopScene::LEVEL_SPECIAL_COURSE) {
                    int currentGameIndex = UserManager::getInstance()->getSpecialCourseCurrentProgress(
                            _levelID, _day);

                    UserManager::getInstance()->setSpecialCourseCurrentProgress(_levelID, _day,
                                                                                currentGameIndex +
                                                                                1);
                }
...
void GameSelectScene::refreshGameButtonForSpecialCourse() {
    ...
    int currentGameIndex = UserManager::getInstance()->getSpecialCourseCurrentProgress(_levelID,
                                                                                       _day);
...
void GameSelectScene::onEnter() {
    ...
    if (cur) {
        ...
        if (UserManager::getInstance()->isGameTestingMode()) {
            ...
            auto shadowScale =
                    bird->getBoundingBox().size.width / _birdShadow->getContentSize().width;
...
//            if (cur->categoryLevel==0) panelFilename = "panel_prek.png";
...

        {
            ...
            auto dayLabel = TodoUtil::createLabel(TodoUtil::itos(_day), 70, Size::ZERO, fontName,
                                                  Color4B(255, 205, 66, 255));
    ...
    if (__firstEnter) {
        ...
        _leavesLeft->setPosition(
                Vec2(-_leavesLeft->getContentSize().width, frameSize.height - 474));
        ...
        _leavesRight->setPosition(Vec2(designSize.width + _leavesLeft->getContentSize().width,
                                       frameSize.height - 382));
        _leavesRight->runAction(Sequence::create(
                DelayTime::create(appearDelay + 0.3),
                EaseOut::create(MoveTo::create(0.2, Vec2(frameSize.width, frameSize.height - 382)),
                                2.0),
                nullptr));
        ...
        if (_bird && _bird->getLevelID() != "" &&
            ((oldLevelID != _bird->getLevelID()) || (oldDay != _day))) {
            spawnGameEggs(0.2);
        } else {
...
void GameSelectScene::checkDayClear() {
    ...
    bool birdActionFlag = (pretestProgressType != PretestProgressType::pass &&
                           pretestProgressType != PretestProgressType::fail) ? true : false;
    ...
    if (cur->categoryLevel == CoopScene::LEVEL_SPECIAL_COURSE) {
        if (UserManager::getInstance()->getSpecialCourseCurrentProgress(levelID, day) >=
            dayCurr->numGames) {
    ...
    if (!UserManager::getInstance()->isDayCleared(levelID, day) || isDayClearSpecialCourse) {
        ...
        if (dayCurr->numGames == numClearedGame) {
            ...
            UserManager::getInstance()->setFishPresentEnable(
                    CurriculumManager::getInstance()->makeLevelID(lang, _bird->getCategory(),
            ...
            if (isDayClearSpecialCourse) {
                if (cur->numDays == dayCurr->dayOrder &&
                    !UserManager::getInstance()->isLevelCleared(levelID)) {
            ...
            auto seq = Sequence::create(
                    ...
                    EaseIn::create(MoveBy::create(0.3, Vec2(0, -30)), 2.0),
                    CallFunc::create(
                            [this, birdActionFlag, reward, dayCurr, isDayClearSpecialCourse, isSpecialCourseClear]() {
                                ...
                                if (isDayClearSpecialCourse) {
                                    _coinTab->addCoinForSpecialCourse(dayCurr->numGames, reward,
                                                                      birdParent->convertToWorldSpace(
                                                                              birdPos),
                                                                      isSpecialCourseClear);

                                } else {
                                    _coinTab->addCoin(reward,
                                                      birdParent->convertToWorldSpace(birdPos),
                                                      dayCurr->isEggQuiz ? false : true);
            ...
            auto seq2 = Sequence::create(EaseOut::create(MoveBy::create(1.6, Vec2(0, 150)), 2.0),
                                         CallFunc::create([this, birdActionFlag, levelID]() {
                                             ...
                                             if (birdActionFlag) {
                                                 _bird->setBirdProgress(
                                                         UserManager::getInstance()->ratioDayCleared(
                                                                 levelID));
                                             ...
                                             auto shadowScale = _bird->getBoundingBox().size.width /
                                                                _birdShadow->getContentSize().width;
                                             _birdShadow->setScale(shadowScale);
                                         ...
                                         CallFunc::create([this, levelID, day]() {
                                             ...
                                             StrictLogManager::shared()->dailyGameChoice_End_Complete(
                                                     levelID, day);

                                             _backButton->setTouchEnabled(true);

                                             ((CustomDirector *) Director::getInstance())->popSceneWithTransition<TransitionFade>(
                                                     0.5);
...
void GameSelectScene::spawnGameEggs(float delay) {
    ...
    this->runAction(Sequence::create(DelayTime::create(1.0 + delay),
                                     CallFunc::create(
                                             []() { SoundEffect::iconSpawnEffect().play(); }),
            ...
            icon->runAction(Sequence::create(
                    ...
                    Spawn::create(
                            ...
                            Sequence::create(
                                    ...
                                    EaseIn::create(
                                            MoveBy::create(0.5, Vec2(0, -120 + (p.y - birdPos.y))),
                                            2.0),
