...
BEGIN_NS_EGGQUIZ ;
    ...
    void EggQuizScene::onEnter() {
        ...
        _progressBar->setPosition(
                Vec2(_winSize.width / 2, _winSize.height - _progressBar->getContentSize().height));
...
// math template
    ...
    void EggQuizScene::createSingleDigitNumbersView() {
        ...
        for (int i = 0; i < _answerInt; i++) {
            ...
            for (int j = 0; j < 10; j++) {
                Vec2 pos = Vec2(random(margin.width + 300,
                                       questionBox->getContentSize().width - margin.width),
                                random(margin.height,
                                       questionBox->getContentSize().height - margin.height));
    ...
    void EggQuizScene::createRecognizeNumberView() {
        ...
        for (auto it : choices) {
            it = TodoUtil::trim(it);
            auto button = PushButton::create(buttonSize, 224, it,
                                             [this](string answer) { checkAnswer(answer); });
    ...
    void EggQuizScene::create2DigitNumbersView() {
...

        {
            ...
            for (int i = 0; i < numSection; i++) {
                ...
                section->setPosition(viewSize.width / 2, top - i * (sectionSize.height + space) -
                                                         sectionSize.height / 2);
    ...
    void EggQuizScene::createBiggerAndSmallerView() {
        ...
        auto addImageToButton = [this](PushButton *button, int count, string imagePath,
                                       float scale = 1) {
            vector<Sprite *> items;
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (lineup) {
                    ...
                    while (1) {
                        newItem->setPosition(
                                Vec2(random < int > (150, 780), random < int > (150, 780)));
        ...
        auto button1 = PushButton::create(buttonSize, 224, choices[0],
                                          [this](string answer) { checkAnswer(answer); });
        button1->setPosition(_gameSize.width / 2 - 32 - buttonSize.width / 2,
                             92 + buttonSize.height / 2);
        _gameNode->addChild(button1);

        auto button2 = PushButton::create(buttonSize, 224, choices[1],
                                          [this](string answer) { checkAnswer(answer); });
        button2->setPosition(_gameSize.width / 2 + 32 + buttonSize.width / 2,
                             92 + buttonSize.height / 2);
        ...
        if (withImage) {
            ...
            addImageToButton(button1, TodoUtil::stoi(choices[0]),
                             partsPath + "pretest_math_dot_yellow.png");
            addImageToButton(button2, TodoUtil::stoi(choices[1]),
                             partsPath + "pretest_math_acorn.png", 0.65);
        }
    ...
    void EggQuizScene::createOperationsWithObjectsView() {
        ...
        for (auto it : fomulaV) {
            if (it[0] == '?') {
                ...
                auto l = TodoUtil::createLabel(it, fomulaNodeHeight, Size::ZERO, FONT_NORMAL,
                                               FONT_COLOR);
        ...
        for (int i = 0; i < ballBox.size(); i++) {
            ...
            switch (it) {
                case 'y':
                    b = Sprite::create(partsPath + "pretest_math_dot_yellow.png");
                    break;
                case 'd':
                    b = Sprite::create(partsPath + "pretest_math_dot_yellow_empty.png");
                    break;
                case 'b':
                    b = Sprite::create(partsPath + "pretest_math_dot_blue.png");
                    break;
            }
    ...
    void EggQuizScene::createOperationsWithoutObjectsView() {
        ...
        if (_problem.questionOption3 == "vertical") {
            ...
            auto num1 = TodoUtil::createLabel(fomulaV[0], 210, Size(600, 210), FONT_NORMAL,
                                              FONT_COLOR, TextHAlignment::RIGHT);
            num1->setPosition(fomulaNodeX, fomulaNodeY);
            fomulaNode->addChild(num1);

            auto num2 = TodoUtil::createLabel(fomulaV[2], 210, Size(600, 210), FONT_NORMAL,
                                              FONT_COLOR, TextHAlignment::RIGHT);
            num2->setPosition(num1->getPositionX(), num1->getPositionY() - lineHeight);
            fomulaNode->addChild(num2);

            auto op = TodoUtil::createLabel(fomulaV[1], 210, Size(600, 210), FONT_NORMAL,
                                            FONT_COLOR, TextHAlignment::RIGHT);
            ...
            fomulaNode->setAnchorPoint(Vec2::ANCHOR_MIDDLE);
            fomulaNode->setPosition(questionBox->getContentSize().width / 2,
                                    questionBox->getContentSize().height / 2 + lineHeight * 3 / 2);
            questionBox->addChild(fomulaNode);
            ...
            for (auto it : fomulaV) {
                if (it[0] == '?') {
                    auto q = Sprite::create(
                            partsPath + "pretest-math-emptyanswer-dotted-large.png");
                    q->setScale(0.8);
                    fomulaNode->addChild(q);
                    q->setPosition(fomulaNodeWidth + q->getContentSize().width / 2, -10);
                    fomulaNodeWidth += q->getContentSize().width + fomulaSpace;
            ...
            fomulaNode->setPosition(questionBox->getContentSize().width / 2,
                                    questionBox->getContentSize().height / 2 +
                                    fomulaNodeHeight / 2);
        }
    ...
    void EggQuizScene::createCompareNumberMagnitudesView() {
        ...
        for (auto it : cardVec) {
            auto button = DragButton::create(it, Size(472, 503), 224, _slots, onTouchMoved,
                                             onTouchEnded);
    ...
    void EggQuizScene::createUseMagnitudeSymbolsView() {
        ...
        auto numberLabel1 = TodoUtil::createLabel(_problem.questionOption1, 224, Size::ZERO,
        ...
        auto numberLabel2 = TodoUtil::createLabel(_problem.questionOption2, 224, Size::ZERO,
                                                  FONT_NORMAL, FONT_COLOR);
    ...
    void EggQuizScene::createShapesView() {
        ...
        if (_problem.questionOption1 == "what") {
            ...
            for (int i = 0; i < choices.size(); i++) {
                ...
                auto button = PushButton::create(buttonSize, fontSize, it, [this](string answer) {
            ...
            for (auto it : choices) {
                auto button = PushButton::create(buttonSize, 100, it,
                                                 [this](string answer) { checkAnswer(answer); });
                button->setPosition(startX, posY);
                button->setImage();
                _gameNode->addChild(button);
    ...
    void EggQuizScene::createNumberIdentificationDragView() {
        ...
        questionBox->setPosition(_gameNode->getContentSize().width / 2, 680);
        _gameNode->addChild(questionBox);
        // convert the number to English words for hi-IN TTS locale
        ...
        float space = 44;
        float startX = _gameSize.width / 2 - (slotSize.width * (_problem.answer.size() - 1) +
                                              space * (_problem.answer.size() - 1)) / 2;
    ...
    void EggQuizScene::createMissingNumberDragView() {
        ...
        for (int i = 0; i < numbers.size(); i++) {
            ...
            if (isSlot) {
                ...
                switch (it.size()) {
                    case 1:
                        slotStartX = slotBoxSize.width / 2;
                        break;
                    case 2:
                        slotStartX = slotBoxSize.width / 2 - 22 - 102;
                        break;
                    case 3:
                        slotStartX = slotBoxSize.width / 2 - 44 - 204;
                        break;
                }
                ...
                for (int i = 0; i < it.size(); i++) {
                    ...
                    slot->setPosition(Vec2(slotStartX, 350 + 312 / 2) + box->getPosition() -
                                      box->getContentSize() / 2);
                    _gameNode->addChild(slot);
                    _slots.push_back(slot);
    ...
    void EggQuizScene::createOperationsDragView() {
        ...
        if (_problem.questionOption3 == "horizontal") {
            ...
            float totalFomulaWidth =
                    label->getContentSize().width + _problem.answer.size() * SlotSizeSmall.width +
                    (_problem.answer.size() - 1) * 24 + 98;
            label->setPosition(
                    _gameSize.width / 2 - totalFomulaWidth / 2 + label->getContentSize().width / 2,
                    891 + 166 / 2);

            float slotStartX = label->getPositionX() + label->getContentSize().width / 2 + 98 +
                               SlotSizeSmall.width / 2;
            ...
            for (auto it : numberVec) {
                ...
                for (int i = (int) it.size() - 1; i > -1; i--) {
                    ...
                    auto label = TodoUtil::createLabel(it2, 224, Size::ZERO, FONT_NORMAL,
                                                       FONT_COLOR);
            ...
            auto delimiterLabel = TodoUtil::createLabel(delimiterStr, 224, Size::ZERO, FONT_NORMAL,
                                                        FONT_COLOR);
            ...
            if (_problem.answer.size() > 1)
                slotStartX -= (SlotSizeSmall.width + 37) * (_problem.answer.size() - 1);
    ...
    void EggQuizScene::createWordProblemView() {
        ...
        float totalSlotWidth =
                _problem.answer.size() * SlotSizeSmall.width + (_problem.answer.size() - 1) * 24;
    ...
    void EggQuizScene::createWordWordView() {
        ...
        Label *questionLabel = TodoUtil::createLabel(_problem.questionOption2, 224, Size::ZERO,
                                                     FONT_NORMAL, FONT_COLOR);
    ...
    void EggQuizScene::createImageWordView() {
        ...
        if (!_problem.questionOption1.empty()) {
            ...
            Label *questionLabel = TodoUtil::createLabel(_problem.questionOption1, 152, Size::ZERO,
                                                         FONT_NORMAL, FONT_COLOR);
            questionBox->addChild(questionLabel);
            ...
            questionLabel->setPosition(
                    posX + widthLimit + margin + questionLabel->getContentSize().width / 2, posY);

        } else {

            Size questionBoxLimitSize = questionBox->getContentSize() * 0.8;
            image->setScale(MIN(questionBoxLimitSize.width / imageSize.width,
                                questionBoxLimitSize.height / imageSize.height));
    ...
    void EggQuizScene::createParagraphSentenceView() {
        ...
        questionBox->setPreferredSize(Size(1136, 1380));
        questionBox->setAnchorPoint(Vec2::ANCHOR_MIDDLE_BOTTOM);
        questionBox->setPosition(_gameNode->getContentSize().width / 2 - 44 -
                                 questionBox->getContentSize().width / 2, 100);
        _gameNode->addChild(questionBox);
        ...
        if (!_problem.questionOption5.empty()) {
            ...
            if (_problem.questionOption5.find(",") != string::npos) {
                ...
                createSmallSpeakerView(ProblemBank::getInstance()->getJoinString(outputV),
                                       Vec2(_gameSize.width / 2 + 200 + 35, 1237));
        ...
        Label *paragraphLabel = TodoUtil::createLabelMultilineToFit(_problem.questionOption2, 80,
                                                                    Size(questionBox->getContentSize().width *
                                                                         0.9,
                                                                         questionBox->getContentSize().height *
                                                                         0.8), FONT_NORMAL,
                                                                    FONT_COLOR,
                                                                    TextHAlignment::LEFT);
        ...
        Label *questionLabel = TodoUtil::createLabelMultilineToFitWidth(_problem.questionOption1,
                                                                        questionLabelWeight ? 88
                                                                                            : 100,
                                                                        Size(1060 -
                                                                             questionLabelWeight,
                                                                             400), FONT_BOLD,
                                                                        ...
                                                                        ? TextHAlignment::LEFT
                                                                        : TextHAlignment::CENTER);
        questionLabel->setPosition(questionLabelWeight ? posX + questionLabelWeight / 2 : posX,
                                   1245 + 215 / 2);
        ...
        for (auto it : choices) {
            it = TodoUtil::trim(it);
            auto button = PushButton::create(buttonSize, 68, it,
                                             [this](string answer) { checkAnswer(answer); });
            button->setPosition(posX, posY);
            _gameNode->addChild(button);
    ...
    void EggQuizScene::createOrderingSentenceView() {
        ...
        for (auto choice : choiceVec) {
            ...
            auto button = DragButton::create(choice, buttonSize, fontSize, _slots, onTouchMoved,
                                             onTouchEnded);
    ...
    void EggQuizScene::showProblem() {
        ...
        if (_problem.templateName == "single_digit_numbers") { createSingleDigitNumbersView(); }
        ...
        else if (_problem.templateName ==
                 "operations_with_objects") { createOperationsWithObjectsView(); }
        else if (_problem.templateName ==
                 "operations_without_objects") { createOperationsWithoutObjectsView(); }
        else if (_problem.templateName ==
                 "biggest_and_smallest") { createBiggestAndSmallestView(); }
        else if (_problem.templateName ==
                 "compare_number_magnitudes") { createCompareNumberMagnitudesView(); }
        else if (_problem.templateName ==
                 "use_magnitude_symbols") { createUseMagnitudeSymbolsView(); }
                 ...
                 "number_identification") { createNumberIdentificationDragView(); }
                ...
                || _problem.templateName == "soundonly_image"
                 ...
                 "listeningcomp_sentence") { createListeningcompSentenceView(); }
    ...
    void EggQuizScene::checkAnswer(string answer) {
        ...
        if (answer == _answer) {
            ...
            StrictLogManager::shared()->eggQuiz_WrongAnswer(_currentProblemIndex, _answer, answer,
                                                            0);
        }
    ...
    void EggQuizScene::onComplete() {
        ...
        if (TodoUtil::startsWith(_level, "fishtest")) {
            ...
            if (cleared) {
                GameSoundManager::getInstance()->playEffectSoundForAutoStart(
                        "Common/Sounds/Effect/pretest_success.m4a");

                if (levelVec.size() > 1)
                    popup->setPostCompleted(_course == "Math" ? 'M' : 'L',
                                            TodoUtil::stoi(levelVec[1]));
                this->runAction(Sequence::create(DelayTime::create(1.0), CallFunc::create(
                        []() { SoundEffect::wowEffect().play(); }), nullptr));
                ...
                GameSoundManager::getInstance()->playEffectSoundForAutoStart(
                        "Common/Sounds/Effect/pretest_fail1.m4a");
                if (levelVec.size() > 1)
                    popup->setPostFailed(_course == "Math" ? 'M' : 'L',
                                         TodoUtil::stoi(levelVec[1]));
                popup->show(this, true, CallFunc::create([this]() {
    ...
    void EggQuizScene::completedFishTest(int score) {
        ...
        auto levelID = CurriculumManager::getInstance()->makeLevelID(lang,
                                                                     _course == "Math" ? 'M' : 'L',
                                                                     CoopScene::LEVEL_FISH_PRESENT);
        ...
        if (gameTestMode) {
            ...
            progressLevelIndex = UserManager::getInstance()->getFishPresentCurrentProgressLevel(
                    levelID);
            progressIndex = UserManager::getInstance()->getFishPresentCurrentProgressIndex(levelID,
                                                                                           progressLevelIndex);

        }

        popup->setFishCompleted(_course == "Math" ? 'M' : 'L', progressLevelIndex, progressIndex,
                                score);
        popup->show(this, true, CallFunc::create([this, levelID, score, gameTestMode]() {
            if (gameTestMode == false && score <= 60 &&
                EggQuiz::EggQuizScene::getTryCountFishTest() == 1) {
                // Retry
                CCAppController::sharedAppController()->handleGameQuit(true);

                string gameName = (this->_course == "Math") ? "EggQuizMath" : "EggQuizLiteracy";
                int progressLevel = UserManager::getInstance()->getFishPresentCurrentProgressLevel(
                        levelID);
                ...
                int progressIndex = UserManager::getInstance()->getFishPresentCurrentProgressIndex(
                        levelID, progressLevel);

                string param = StringUtils::format("fishtest_%d_%d", progressLevel + 1,
                                                   (progressIndex % setCount) + 1);
                CCAppController::sharedAppController()->startGame(gameName,
                                                                  CoopScene::LEVEL_FISH_PRESENT,
                                                                  param);
...
// sub template
    ...
    void EggQuizScene::createPushButton09Template() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            auto button = PushButton::create(BUTTON_SIZE_SMALL, 200, choice,
                                             [this, choice](string answer) {
                                                 checkAnswer(answer);
                                             });
    ...
    void EggQuizScene::createDragButton09Template() {
        ...
        for (int i = 0; i < 10; i++) {
            auto button = DragButton::create(TodoUtil::itos(i), BUTTON_SIZE_SMALL, 200, _slots,
                                             onTouchMoved, onTouchEnded);
    ...
    void EggQuizScene::createAnswerNumberPadView() {
        ...
        Label *answerLabel = TodoUtil::createLabel(_padNumber, 220, Size(750, 190), FONT_NORMAL,
                                                   FONT_COLOR, TextHAlignment::RIGHT);
        ...
        auto panelPos = [panelSize, rowCount, colCount, panelAreaMargin](int row, int col) {
            ...
            float y = (10.f + panelSize.height) * (rowCount - row - 1) + panelSize.height / 2 +
                      panelAreaMargin;
        ...
        for (int i = -2; i <= 9; i++) {
            ...
            if (i == -1) {
                btn = ui::Button::create(partsPath + "pretest_math_calculator_button_normal.png",
                                         partsPath + "pretest_math_calculator_button_active.png");
                ...
                btn = ui::Button::create(
                        partsPath + "pretest_math_calculator_button_enter_normal.png",
                        partsPath + "pretest_math_calculator_button_enter_active.png");
                btnLabel = Label::create();
                btnPos = panelPos(rowCount - 1, 2);
            } else {
                btn = ui::Button::create(partsPath + "pretest_math_calculator_button_normal.png",
                                         partsPath + "pretest_math_calculator_button_active.png");
                btnLabel = TodoUtil::createLabel(TodoUtil::itos(i), fontSize, Size::ZERO,
                                                 FONT_NORMAL, FONT_COLOR);
                if (i == 0) {
            ...
            btn->addTouchEventListener(
                    [btn, btnLabel, normalColor, highlightColor, this](Ref *sender,
                                                                       Widget::TouchEventType event) {
    ...
    void EggQuizScene::createPushButtonC3WTemplate(float fontSize) {
        ...
        for (auto it : choices) {
            it = TodoUtil::trim(it);
            auto button = PushButton::create(buttonSize, fontSize, it,
                                             [this](string answer) { checkAnswer(answer); });
            button->setPosition(startX, posY);
            _gameNode->addChild(button);
            startX += space + buttonSize.width;
            ...
            if (it.find("+") != string::npos) {
                ...
                for (auto ch : chars) {
                    ...
                    auto label = TodoUtil::createLabel(ch, 152, Size::ZERO, FONT_NORMAL,
                                                       FONT_COLOR);
                    label->setAnchorPoint(Vec2::ANCHOR_MIDDLE);
                ...
                image->setScale(MIN(buttonLimitSize.width / imageSize.width,
                                    buttonLimitSize.height / imageSize.height));
        ...
        for (auto sequence : sequenceV) {
            ...
            if (sequence == "?") {
                ...
                if (sequence.find(".png") != string::npos) {
                    ...
                    image->setScale(
                            MIN(frame->getContentSize().width / image->getContentSize().width,
                                frame->getContentSize().height / image->getContentSize().height) *
                            0.88);
                    frame->addChild(image);

                } else {
                    auto text = TodoUtil::createLabelMultilineToFit(sequence, 64 * 0.7,
                                                                    Size(438, 288), FONT_NORMAL,
                                                                    FONT_COLOR);
