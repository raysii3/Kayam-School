...
Node* createBird(char category, int level, Size winSize, bool isSpecialTest)
{
    if (isSpecialTest) level = 10;
    string name;
    ...
    auto bird = Sprite::create(partsPath+name+".png");
...
Node* createBirdFailed(char category, int level, Size winSize, bool isSpecialTest)
{
    if (isSpecialTest) level = 10;
    string eggName;
...
void EggQuizPopup::setConfirmQuit(char category, int level)
{
    ...
    {
        auto btn = createButton(partsPath+"popup_button_test_exit", "Stop the test");
    ...
    {
        auto btn = createButton(partsPath+"popup_button_test_backtotest", "Go back to test");
...
void EggQuizPopup::setPreCompleted(char category, int level)
{
    ...
    {
        auto sp = Sprite::create(partsPath+"popup_window_ribbon_back.png");
    ...
    {
        auto sp = Sprite::create(partsPath+"popup_window_ribbon_front.png");
        sp->setPosition(winSize.width/2, winSize.height-750);
        this->addChild(sp);
...
void EggQuizPopup::setPostCompleted(char category, int level)
{
    if (!_isSpecialTest) {
        ...
        if (UserManager::getInstance()->getPretestProgressType(_cur->levelID) != PretestProgressType::finish) {
    ...
    {
        auto b = createBird(category, level, winSize, _isSpecialTest);
...
void EggQuizPopup::setPreFailed(char category, int level)
{
    ...
    {
        auto btn = createButton(partsPath+"pretest_button_back", "");
...
void EggQuizPopup::setPostFailed(char category, int level)
{
    ...
    {
        auto b = createBirdFailed(category, level, winSize, _isSpecialTest);
        b->setPosition(winSize.width/2, winSize.height-750);
        this->addChild(b);
    }
